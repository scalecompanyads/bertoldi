...
export function ProcessoCard({ processo: p, href, ultimaVerificacao, novidade, clienteNome, compacto }: Props) {
  ...
  return (
    <Link
      href={href}
      className={`block rounded-2xl border bg-card shadow-sm hover:shadow-md hover:bg-accent transition-all duration-150 ${
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 space-y-2">
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`${compacto ? 'text-[11px]' : 'text-xs'} px-2.5 py-0.5 rounded-full font-medium ${STATUS_COR[p.status_interno]}`}>
              {STATUS_PROCESSO_LABEL[p.status_interno]}
            </span>
            {novidade && (
              <span className="text-[11px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full bg-emerald-400/10 text-emerald-600 dark:text-emerald-300">
                Novidade
              </span>
            )}
          </div>
          <p className={`font-semibold leading-snug ${compacto ? 'text-sm line-clamp-2' : 'text-base'}`}>
            {titulo}
        ...
        >
          {detalhes.map((d) => (
            <div key={d.label} className="min-w-0">
              <dt className="text-[10px] uppercase tracking-wider text-muted-foreground/70 font-medium">{d.label}</dt>
